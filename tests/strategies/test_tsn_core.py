...
class TestTSNCoreStrategy:
    ...
    def test_capacity_exhaustion_raises(self, simple_trajectories):
        """Single-copy strategies must fail loudly once mask capacity is exhausted."""
        from tsn_affinity.core.exceptions import StrategyError
        ...
        trajs_0 = simple_trajectories(
            n_trajs=2, traj_len=20, obs_dim=4, n_actions=2, seed=0
        )
        strategy.train_task(trajs_0, steps=5, batch_size=2)
        strategy.after_task(trajs_0)

        trajs_1 = simple_trajectories(
            n_trajs=2, traj_len=20, obs_dim=4, n_actions=2, seed=100
        )
        with pytest.raises(StrategyError):
            strategy.train_task(trajs_1, steps=5, batch_size=2)
