...
from tsn_affinity.sparse.converter import iter_sparse_modules
        ...
        with torch.no_grad():
            for name, mod in iter_sparse_modules(model):
                ...
                if src_w is not None:
                    ...
                    mod.score.normal_(mean=0.0, std=self.noise_std)  # type: ignore[operator]
                    mod.score.add_(self.strength * src_w)  # type: ignore[operator]
                ...
                if getattr(mod, "bias_score", None) is not None:
                    ...
                    if src_b is not None:
                        ...
                        mod.bias_score.normal_(mean=0.0, std=self.noise_std)  # type: ignore[operator]
                        mod.bias_score.add_(self.strength * src_b)  # type: ignore[operator]
