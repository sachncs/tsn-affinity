...
This module exposes the metric helpers that the routing strategies use
to decide whether to spawn a new model copy or reuse an existing one.

Two families of metrics are supported:

    - **Cross-entropy affinity** for discrete actions.
    - **Mean-squared-error affinity** for continuous actions.

The :func:`compute_action_affinity` and
:func:`compute_action_affinity_batch` helpers return the appropriate
metric based on the dimensionality of the actions they receive. They
do not conflate the two into a single value.
"""
...
AffinityMetric = Literal["auto", "cross_entropy", "mse"]
...
def compute_action_affinity_batch(
    ...
    Args:
        ...
        metric: Affinity metric to use. ``"auto"`` (default) picks
            cross-entropy for ``actions.dim() == 2`` (discrete) and
            mean-squared-error for ``actions.dim() == 3`` (continuous).
            Pass ``"cross_entropy"`` or ``"mse"`` to force a metric,
            which raises :class:`ValueError` if the dimensions do not
            match.

    Returns:
        Dict mapping task_id -> mean affinity score (lower = better).

    Raises:
        ValueError: If ``metric`` is forced but mismatches the
    ...
    sample_actions = task_trajs[0].actions
    if metric == "auto":
        affinity_kind = "mse" if sample_actions.ndim == 2 else "cross_entropy"
    else:
        affinity_kind = metric

    if affinity_kind == "cross_entropy" and sample_actions.ndim != 1:
        raise ValueError(
            "cross_entropy affinity requires discrete (1-D) actions, got "
            f"ndim={sample_actions.ndim}"
        )
    if affinity_kind == "mse" and sample_actions.ndim != 2:
        raise ValueError(
            "mse affinity requires continuous (2-D) actions, got "
            f"ndim={sample_actions.ndim}"
        )

    loader = _MultiBatchLoader(task_trajs, seq_len, batch_size, device, n_batches)

    task_scores: dict[int, list[float]] = {tid: [] for tid in task_ids}
    ...
    with torch.no_grad():
        for batch_data in loader:
            ...
            for tid in task_ids:
                ...
                if affinity_kind == "mse":
                    mse = F.mse_loss(logits, actions, reduction="none").mean(dim=-1)
                    score = (mse * mask).sum() / mask.sum().clamp(min=1.0)
                else:
                ...
                task_scores[tid].append(float(score.detach().cpu().item()))

    return {
        tid: float(np.mean(scores)) if scores else float("inf")
...
def compute_action_affinity(
    ...
    Computes either mean cross-entropy (discrete actions) or mean
    squared error (continuous actions) between the model's predictions
    ...
    Args:
        ...
        batch_size: Batch size for affinity estimation.
        device: Device to run computation on.
        metric: ``"auto"`` (default) picks the metric from the action
            shape, ``"cross_entropy"`` forces cross-entropy, and
            ``"mse"`` forces mean squared error.

    Returns:
        Mean affinity score (lower = better compatibility).
...
def make_temp_minibatches(
    ...
    while True:
        ...
        yield obs, actions, rtg_tensor, ts_tensor


def unpack_temp_batch(
    batch: tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor],
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """Unpack temporary batch.

    Args:
        batch: Tuple from :func:`make_temp_minibatches`.

    Returns:
        Tuple of ``(obs, actions, rtg, ts, mask)`` where ``mask`` is
        all-ones where ``actions != -1``.
    """
