...
class TrainingSnapshot:
    ...
    Each entry stores the parameter value alongside an optional
    restore mask. The mask is ``None`` for parameters that should be
    fully restored and a binary mask for parameters that should only
    be restored where the mask is true.

    Attributes:
        name_to_value_and_mask: Dict of param_name to (value, mask).
    """

    name_to_value_and_mask: dict[str, tuple[torch.Tensor, torch.Tensor | None]]


def verify_frozen_gradient_zeroing(
    model: torch.nn.Module,
    consolidated_masks: dict[str, torch.Tensor | None],
...
def snapshot_frozen_parameters(
    ...
    """
    snap: dict[str, tuple[torch.Tensor, torch.Tensor | None]] = {}
...
def zero_gradients_for_frozen_params(
    ...
    for key, old_mask in consolidated_masks.items():
        ...
        if param is None or param.grad is None:
            continue
        mask = old_mask.to(
            device=param.grad.device, dtype=torch.bool, non_blocking=True
        )
