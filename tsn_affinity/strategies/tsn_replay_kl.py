...
ReplayKLRoutingConfig = RoutingConfig
...
class TSNReplayKLStrategy(TSNBaseStrategy):
    ...
    The strategy uses the same ``RoutingConfig`` dataclass as
    :class:`TSNAffinityStrategy`. Only the ``mode`` (``"replay_kl"``),
    ``kl_threshold``, ``memory_size``, and ``max_copies`` fields are
    read.
    """
    ...
    def __init__(
        ...
        if routing_config is None:
            routing_config = RoutingConfig(mode="replay_kl")
    ...
    def _activate_copy(self, copy_id: int) -> None:
        ...
        # Restore per-task and consolidated masks from the activated
        # copy so successive calls to ``set_eval_task`` operate on the
        # right state.
        ...
        self.task_keep_ratios = state.task_keep_ratios
    ...
    def _sync_public_state_to_active_copy(self) -> None:
        ...
        state.task_keep_ratios = self.task_keep_ratios
