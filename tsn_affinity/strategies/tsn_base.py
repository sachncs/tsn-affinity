...
from tsn_affinity.core.exceptions import ConfigurationError, StrategyError
...
class TSNBaseStrategy(BaseStrategy):
    ...
    def _recompute_module_masks(
        ...
        with torch.no_grad():
            # `current_weight_mask` and `current_bias_mask` live on
            # the sparse mixin; mypy sees only nn.Module here. We
            # access them dynamically so we do not pin this method to
            # the mixin's static type.
            if hasattr(mod, "weight_mask"):
                mod.weight_mask = None  # type: ignore[assignment]
            if hasattr(mod, "bias_mask"):
                mod.bias_mask = None  # type: ignore[assignment]

            w_mask = mod.current_weight_mask()  # type: ignore[operator]
            if getattr(mod, "bias", None) is not None:
                b_mask: torch.Tensor | None = mod.current_bias_mask()  # type: ignore[operator]
            else:
                b_mask = None
    ...
    def _compute_task_keep_ratio(self) -> float:
        """Compute the per-task keep ratio respecting current mask occupancy.

        Returns ``1.0`` (no sparsity) when no free capacity remains and
        raises :class:`ConfigurationError` when the requested keep ratio
        cannot be honoured without crossing the capacity limit, instead
        of silently saturating the mask.

        Returns:
            The keep ratio to use for the upcoming task, clipped to
            ``[1e-3, 1.0]``.

        Raises:
            ConfigurationError: When ``free_ratio`` is too small to fit
                the requested ``keep_ratio`` and the value would be
                saturated above the configured ``sparse_config.keep_ratio``.
        """
        ...
        if free_ratio <= 1e-6:
            raise ConfigurationError(
                "All mask capacity is consumed by previous tasks; "
                "create a new model copy or relax the keep_ratio "
                "before training the next task."
            )

        task_keep_ratio = base / free_ratio

        if task_keep_ratio >= 1.0 - 1e-6:
            raise ConfigurationError(
                f"Requested per-task keep_ratio={base:.4f} would saturate "
                f"the mask (free_ratio={free_ratio:.4f}). Increase the "
                "total capacity (e.g., create a new copy) or lower "
                "keep_ratio."
            )
    ...
    def _prepare_current_task(self) -> None:
        ...
        try:
            ...
            raise StrategyError(
                "Failed to prepare the current task because mask "
                "capacity was exhausted. Use a multi-copy strategy or "
                "lower the configured keep_ratio."
            ) from exc
