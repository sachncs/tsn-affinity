...
from tsn_affinity.core.exceptions import RoutingError
...
# Backwards-compatible alias. New code should import ``RoutingConfig``
# directly from ``tsn_affinity.core.config``.
AffinityRoutingConfig = RoutingConfig
...
class TSNAffinityStrategy(TSNBaseStrategy):
    ...
    def __init__(
        ...
        affinity_config: RoutingConfig | None = None,
        seed: int | None = None,
    ) -> None:
        ...
        self.warmstarter = MaskWarmstarter(
            warmstart=bool(affinity_config.warmstart),
            strength=float(affinity_config.warmstart_strength),
            noise_std=float(affinity_config.warmstart_noise_std),
        )
    ...
    def _compute_current_memory_latent_stats(
        self, task_memory_obs: torch.Tensor
    ...
    def _activate_copy(self, copy_id: int) -> None:
        ...
        self.consolidated_masks = state.consolidated_masks
        self.per_task_masks = state.per_task_masks
        self.task_codebooks = state.task_codebooks
    ...
    def _sync_public_state_to_active_copy(self) -> None:
        ...
        state.consolidated_masks = self.consolidated_masks
        state.per_task_masks = self.per_task_masks
        state.task_codebooks = self.task_codebooks
    ...
    def _select_copy_for_new_task(
    ...
    ) -> tuple[int, int | None, dict, bool]:
        if self.current_task_id == 0 or not self.task_to_copy:
            return (
                0,
                None,
                {"best_action": None, "best_latent": None, "best_score": None},
                False,
            )
        ...
        if self.affinity_config.mode == "action":
            ...
            if len(previous_tasks) == 1:
                final_scores = {
                    previous_tasks[0]: compute_hybrid_affinity(
                ...
                act_norm = (
                    normalize_scores(action_scores)
                    if self.affinity_config.normalize_scores
                    else action_scores
                )
                lat_norm = (
                    normalize_scores(latent_scores)
                    if self.affinity_config.normalize_scores
                    else latent_scores
                )
                final_scores = {
            ...
            raise RoutingError(
                f"Unsupported routing mode: {self.affinity_config.mode!r}"
            )

        # Validate that no score is non-finite before proceeding; this
        # prevents silently routing to the first copy when affinity
        # computation produced NaN values.
        self._validate_affinity_scores(action_scores)
        if latent_scores:
            self._validate_affinity_scores(latent_scores)
        self._validate_affinity_scores(final_scores)

        best_task = min(final_scores, key=final_scores.get)  # type: ignore[arg-type]
        ...
        if self.affinity_config.relative_threshold and len(all_scores) > 1:
            if score_range > 1e-6:
                ...
                create_new = relative_margin < (
                    1.0 / self.affinity_config.copy_creation_margin
                )
        else:
            if self.affinity_config.mode == "action":
        ...
        # Respect max_model_copies when set.
        if (
            create_new
            and self.affinity_config.max_model_copies is not None
            and len(self.copy_states) >= self.affinity_config.max_model_copies
        ):
            create_new = False
            best_task = int(best_task)
            best_score = final_scores[best_task]
            relative_margin = 0.0
    ...
    def _validate_affinity_scores(self, scores: dict[int, float]) -> None:
        """Reject affinity scores containing NaN or non-finite values.

        Args:
            scores: Map of task ID to affinity score.

        Raises:
            RoutingError: If any score is non-finite.
        """
        for task_id, score in scores.items():
            if score is None or not np.isfinite(score):
                raise RoutingError(
                    f"Non-finite affinity score for task {task_id}: {score!r}"
                )
    ...
    def train_task(
        ...
        if self.current_task_id == 0:
            ...
            copy_id, src_task, score_details, created_new = (
                self._select_copy_for_new_task(
                    task_memory,
                    task_trajs,
                )
            )
        ...
        if src_task is not None:
            self.warmstarter.apply(
                self.model, src_task, self.task_to_copy, self.copy_states
            )
        ...
        for iteration in range(int(steps)):
            ...
            if iteration % 20000 == 0 or iteration == int(steps) - 1:
                ...
                logging.getLogger("tsn_affinity").info(
                    "[tsn-affinity] task=%s copy=%s src=%s "
                    "mode=%s best_action=%s best_latent=%s "
                    "best_score=%s new_copy=%s iteration=%s "
                    "ce=%.6e keep_ratio=%.4f",
    ...
    def after_task(self, task_trajs: list[Trajectory]) -> None:
        ...
        logging.getLogger("tsn_affinity").info(
            "[tsn-affinity] after task %s: copy=%s "
            "occupied_ratio=%.4f source_task=%s "
            "best_action=%s best_latent=%s best_score=%s "
            "created_new_copy=%s",
            task_id,
            self.current_copy_id,
            ratio,
            meta.get("source_task"),
            meta.get("best_action"),
            meta.get("best_latent"),
            meta.get("best_score"),
            meta.get("created_new_copy"),
        )
