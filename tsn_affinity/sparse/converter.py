    ...
    for name, child in list(model.named_children()):
        ...
        if isinstance(child, nn.Linear) and not isinstance(child, TSNSparseLinear):
            ...
            new_child = TSNSparseConv2d(
                child.in_channels,
                child.out_channels,
                tuple(child.kernel_size),  # type: ignore[arg-type]
                stride=tuple(child.stride),  # type: ignore[arg-type]
                padding=tuple(child.padding),  # type: ignore[arg-type]
                dilation=tuple(child.dilation),  # type: ignore[arg-type]
                groups=child.groups,
                bias=(child.bias is not None),
...
def kmeans_quantize(
    ...
    actual_clusters = int(min(max(1, n_clusters), n_selected, len(unique_vals)))
    if actual_clusters < 2:
        return None

    kmeans = KMeans(n_clusters=actual_clusters, n_init=4, random_state=0)
    labels = kmeans.fit_predict(selected)
    centers: np.ndarray = kmeans.cluster_centers_.reshape(-1)
