...
class _EvalBatchLoader:
    ...
    def _prebuild_batches(self) -> None:
        self.batches = []
    ...
    def __iter__(self) -> "_EvalBatchLoader":
        self._current = 0
        return self
...
def make_strategy(
    ...
    Args:
        name: Strategy name (``"tsn_core"``, ``"tsn_affinity"``, or
            ``"tsn_replay_kl"``).
        obs_shape: Observation shape.
        n_actions: Number of discrete actions.
        device: Torch device string.
        seed: Random seed used by the strategy for reproducibility.

    Returns:
        A configured continual-learning strategy.
    """
    if name == "tsn_core":
        ...
        return TSNReplayKLStrategy(
            ...
            model_config=ModelConfig(obs_shape=obs_shape, n_actions=n_actions),
            sparse_config=SparseConfig(keep_ratio=0.3),
            routing_config=RoutingConfig(mode="replay_kl", seed=seed),
            seed=seed,
        )
    else:
