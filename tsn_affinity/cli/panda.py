...
The Panda adapter expects data in the ``panda-gym`` offline pickle format.
Until ``panda-gym`` is installed in your environment, this CLI will raise
a helpful ``ImportError`` pointing at the missing dependency.

Usage:
    python -m tsn_affinity.cli.panda --data data/panda_tasks.pkl --strategy tsn_affinity
"""
...
def run_panda_benchmark(
    ...
    if strategy_name == "tsn_affinity":
        affinity_config = RoutingConfig(mode="hybrid", hybrid_alpha=0.5, seed=seed)
