"""Panda-specific data loading utilities."""
...
def make_minibatches_panda(
    ...
    Identical logic to make_minibatches but may be extended for Panda-specific
    preprocessing (e.g., action masking for robot joints).
    ...
    while True:
        ...
        actions = torch.tensor(np.stack([b[1] for b in batch]), dtype=torch.float32, device=device)
