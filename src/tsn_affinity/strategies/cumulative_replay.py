...
class CumulativeReplayStrategy(TSNBaseStrategy):
    ...
    def __init__(
        ...
        if sparse_config is None:
            sparse_config = SparseConfig(keep_ratio=replay_ratio)
    ...
    def train_task(
        ...
        if current_steps > 0:
            ...
            for iteration in range(current_steps):
                ...
                if iteration % 500 == 0 or iteration == current_steps - 1:
                    ...
                    print(
                        f"[cumulative-replay] task={self.current_task_id} "
                        f"iteration={iteration}/{current_steps} loss={avg_loss:.6f}"
                    )
        ...
        if replay_steps > 0 and self.replay_buffer:
            ...
            for iteration in range(replay_steps):
        ...
        return {"loss": total_loss / max(1, steps), "keep_ratio": float(self.current_keep_ratio)}
    ...
    def after_task(self, task_trajs: List[Trajectory]) -> None:
        ...
        print(f"[cumulative-replay] after task {task_id}: occupied_ratio={ratio:.4f}")
