    ...
    with torch.no_grad():
        for batch_data in loader:
            ...
            stacked_logits = torch.stack([
                models[model_idx_map[tid]](obs, actions, rtg, ts, attention_mask=mask)
                for tid in task_ids
            ])
...
class _MultiBatchLoader:
    ...
    def _generate_batches(
        ...
        for _ in range(n_batches):
            ...
            act_t = torch.tensor(np.stack(batch_act), dtype=torch.long, device=device)
...
class _AffinityBatchLoader:
    ...
    def next_batch(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
