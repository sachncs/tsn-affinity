...
from tsn_affinity.core.attention import Block, CausalSelfAttention, LayerNorm, MLP
...
class DTBackbone(nn.Module):
    ...
    def __init__(
        ...
        self.state_embed = None  # Set by set_obs_encoder
    ...
    def forward(
        ...
        Args:
            ...
            action: [B, K] action indices (teacher forcing during training).
            timesteps: [B, K] timestep indices.
            attention_mask: [B, K] binary mask (1 = valid, 0 = pad).

        Returns:
            Action logits [B, K, n_actions] at state positions.
        """
        B, K = action.shape

        # Embed all modalities - rtg is [B, K, 1], state is [B, K, ...], action is [B, K]
        rtg_emb = self.rtg_embed((rtg / self.rtg_scale).reshape(B * K, 1)).reshape(B, K, self.d_model)

        if self.state_embed is not None:
            if len(state.shape) == 5:
                state_enc = self.state_embed(state.reshape(B * K, *state.shape[2:])).reshape(B, K, self.d_model)
            elif len(state.shape) == 4:
                state_enc = self.state_embed(state.reshape(B * K, *state.shape[2:])).reshape(B, K, self.d_model)
            elif len(state.shape) == 3:
                state_enc = self.state_embed(state.reshape(B * K, -1)).reshape(B, K, self.d_model)
            else:
        ...
        x = torch.stack([rtg_emb, state_enc, action_emb], dim=2).reshape(B, K * 3, self.d_model)
        ...
        return self.action_head(state_logits)
...
class DecisionTransformer(nn.Module):
    ...
    def forward(
        ...
        return self.backbone(returns_to_go, obs, actions, timesteps, attention_mask)
    ...
    def act(
        ...
        act_seq = torch.tensor(act_history, device=device).unsqueeze(0).long()  # [1, K]
        rtg_seq = torch.tensor(rtg_history, device=device).unsqueeze(0).unsqueeze(-1)  # [1, K, 1]
        ts_seq = torch.tensor(ts_history, device=device).unsqueeze(0).long()  # [1, K]

        # Pad observation sequence to full seq_len at the beginning
        # The sequence should have seq_len positions, with recent observations at the end
        pad_len = self.seq_len - len(obs_history)
        if pad_len > 0:
            obs_pad = torch.zeros(pad_len, *obs_seq.shape[1:], device=device, dtype=obs_seq.dtype)
            obs_seq = torch.cat([obs_pad, obs_seq], dim=0)

        # Action, RTG, and timestep sequences should all be padded to seq_len
        # even when act_history is empty (first step)
        # Use 0 for padded actions (not -1, as -1 is invalid for embedding lookup)
        if len(act_history) > 0:
            act_seq = torch.nn.functional.pad(act_seq, (0, self.seq_len - act_seq.shape[1]), value=0)
        else:
            act_seq = torch.zeros(1, self.seq_len, device=device, dtype=torch.long)

        if len(rtg_history) > 0:
            rtg_seq = torch.nn.functional.pad(rtg_seq, (0, 0, 0, self.seq_len - rtg_seq.shape[1]), value=0.0)
        else:
            rtg_seq = torch.zeros(1, self.seq_len, 1, device=device, dtype=rtg_seq.dtype)

        if len(ts_history) > 0:
            ts_seq = torch.nn.functional.pad(ts_seq, (0, self.seq_len - ts_seq.shape[1]), value=0)
        else:
            ts_seq = torch.zeros(1, self.seq_len, device=device, dtype=torch.long)

        mask = torch.ones(1, self.seq_len, device=device)
        # Add batch dimension to obs_seq: [K, C, H, W] -> [1, K, C, H, W]
